...
{
    private Dictionary<string, string> namespaceMapping = new();
    ...
    public LinqToXsdSettings Load(string configFile)
    {
        if (string.IsNullOrEmpty(configFile))
            throw new ArgumentException("Argument configFile should be non-null and non-empty.");

        return Load(XDocument.Load(configFile));
    }

    public LinqToXsdSettings Load(XDocument configDocument)
    {
