...
<schema xmlns="http://www.ascc.net/xml/schematron">
	<title>EML-UK 470 - VToken Log</title>
	...
	<pattern name="eml">
		...
		<rule context="eml:EML">
			<assert id="3000-004" test="@Id='470'">The value of the Id attribute of the EML element is incorrect</assert>
			<assert id="3000-005" test="eml:VTokenLog">The message type must match the Id attribute of the EML element</assert>
		</rule>
	</pattern>
	...
	<pattern name="eml-470">
		<rule context="eml:EML[eml:SequenceNumber]">
			<assert id="3470-001" test="eml:SequencedElementName='VToken' or eml:SequencedElementName='VTokenQualified'">The sequenced element name is incorrect</assert>
		</rule>
	</pattern>

	<pattern name="eml-470-uk">
		<rule context="eml:VTokenLog">
			<assert id="4470-001" test="eml:ElectionIdentifier">The election identifier is mandatory</assert>
		</rule>
		<rule context="eml:VTokens">
			<assert id="4470-002" test="eml:VotingChannel">The voting channel is mandatory</assert>
		</rule>
	</pattern>
</schema>
